...
pub fn flatterField(data: anytype, comptime field_name: []const u8) FlattenedType {
    const field = getField(data, field_name);
    const TField = @TypeOf(field);
    comptime debug.assert(@sizeOf(TField) <= @sizeOf(FlattenedType));
...
pub fn unflatterField(comptime TDest: type, data: FlattenedType) TDest {
    comptime debug.assert(@sizeOf(TDest) <= @sizeOf(FlattenedType));
...
fn FieldRef(comptime T: type, comptime field_name: []const u8) type {
    comptime {
        ...
        debug.assert(TField != comptime_int);
        debug.assert(TField != comptime_float);
        debug.assert(TField != @TypeOf(.Null));
