...
pub fn nextLevel(self: *Self) Allocator.Error!void {
...
pub fn endLevel(self: *Self) ParseErrors!void {
    var current_level = self.current_level;
    var prev_level = current_level.parent orelse return ParseErrors.UnexpectedCloseSection;
    var last_node = prev_level.peek() orelse return ParseErrors.UnexpectedCloseSection;
...
pub fn endRoot(self: *Self) ParseErrors![]Node {
    if (self.current_level != self.root) {
        return ParseErrors.UnexpectedEof;
    }
...
pub fn addNode(self: *Self, part_type: PartType, text_part: *const TextPart) Allocator.Error!void {
