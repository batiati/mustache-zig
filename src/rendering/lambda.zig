...
///
/// Returns true if TValue is a type generated by LambdaContextImpl(...) 
pub fn isLambdaContextImpl(comptime TValue: type) bool {
    return @typeInfo(TValue) == .Struct and
        @hasField(TValue, "out_writer") and
        @hasField(TValue, "data") and
        @hasField(TValue, "bound_fn") and
        blk: {
        const Writer = meta.fieldInfo(TValue, .out_writer).field_type;
        ...
        break :blk comptime isValidLambdaFunction(TData, TFn) and
            TValue == LambdaContextImpl(Writer, TFn, TData);
    };
}

test "isLambdaContextImpl" {
    const foo = struct {
    ...
    const Impl = LambdaContextImpl(Writer, TFn, void);
    const IsntImpl = struct { field: usize };

    try testing.expect(isLambdaContextImpl(Impl));
    try testing.expect(isLambdaContextImpl(IsntImpl) == false);
    try testing.expect(isLambdaContextImpl(TFn) == false);
    try testing.expect(isLambdaContextImpl(u32) == false);
}
...
pub fn LambdaContextImpl(comptime Writer: type, comptime TFn: type, comptime TData: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn invoke(self: *const Self, allocator: Allocator, tag_contents: []const u8) anyerror!void {
...
test "LambdaContextImpl" {

    // LambdaContextImpl is comptime validated
    ...
    {
        const Impl = LambdaContextImpl(Writer, @TypeOf(Foo.staticFn), void);
        var impl = Impl{ .out_writer = std.io.null_writer, .bound_fn = Foo.staticFn, .data = {} };

        const last_counter = Foo.static_counter;
        try impl.invoke(undefined, undefined);
    ...
    {
        const Impl = LambdaContextImpl(Writer, @TypeOf(Foo.selfFnValue), Foo);
        var foo = Foo{};
        var impl = Impl{
            .out_writer = std.io.null_writer,
    ...
    {
        const Impl = LambdaContextImpl(Writer, @TypeOf(Foo.selfFnConstPtr), *Foo);
        var foo = Foo{};
        var impl = Impl{
            .out_writer = std.io.null_writer,
    ...
    {
        const Impl = LambdaContextImpl(Writer, @TypeOf(Foo.selfFnPtr), *Foo);
        var foo = Foo{};
        var impl = Impl{
            .out_writer = std.io.null_writer,
