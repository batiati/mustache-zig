        ...
        pub const DataRender = struct {
                ...
                while (level) |current| : (level = current.parent) {
                    ...
                    switch (path_resolution) {
                        ...
                        .lambda => {
                            ...
                            // Expand the lambda against the current context and break the loop
                            const expand_result = try current.ctx.expandLambda(self, path, "", escape, .{});
                            if (expand_result != .lambda and self.stack_global_lambdas != null) {
                                std.debug.print ("GLOBAL LAMBDAS\n", .{});
            ...
            fn pathCapacityHint(
                ...
                while (level) |current| : (level = current.parent) {
                    ...
                    std.debug.print ("PATH RES = {s}\n", .{@tagName(path_resolution)});
                    ...
                    switch (path_resolution) {
                        ...
                        .not_found_in_context => {
                            // Not rendered, should try against the parent context
                            if (self.stack_global_lambdas) |global_lambdas| {
                                var level_global_lambdas: ?*const ContextStack = global_lambdas;
            ...
            pub fn valueCapacityHint(
                ...
                std.debug.print ("valueCapacityHint\n", .{});
                switch (@typeInfo(TValue)) {
                    .Bool => return 5,
                    .Int,
                    .ComptimeInt,
...
// TODO: commented for faster compilation time. Uncomment when done
//const tests = struct {
//    const spec = struct {
//        test {
//            _ = interpolation;
